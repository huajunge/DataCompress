...
public class Simple8b implements IIntegerCompress {
     ...
     * 特殊的bits
     */
    private int[] specialBits = new int[]{10, 12, 15, 20, 30, 60};
    ...
    private int findSelectorValue(int currentBits) {
        ...
        if (currentBits <= specialBits[specialIndex]) {
            return 11;
        } else if (specialBits[specialIndex++] < currentBits && currentBits <= specialBits[specialIndex]) {
            return 12;
        } else if (specialBits[specialIndex++] < currentBits && currentBits <= specialBits[specialIndex]) {
            return 13;
        } else if (specialBits[specialIndex++] < currentBits && currentBits <= specialBits[specialIndex]) {
